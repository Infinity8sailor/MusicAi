...
// import { Player } from "../hawler_player";
...
export const walkSync = function (dir, filelist = null) {
...
async function parseFiles(audioFiles) {
  ...
  for (const audioFile of audioFiles) {
    ...
    var data = { title: "", artist: "", modDate: new Date() };
...
export async function scanDir(filePath) {
  if (!filePath) return;
  ...
  let arg = { files: null, path: null, names: null };
  ...
  // startPlayer(arg);
  console.log("from scanDir", arg.files.length);
  storage.set("saved-data", arg, function (error) {
...
export function startPlayer(arg) {
  if (songPlaying) {
    player.pause();
    songPlaying = false;
  }
  ...
  storage.has("last-played", function (error, hasKey) {
    ...
    if (hasKey) {
      storage.get("last-played", function (error, data) {
        if (error) throw error;
        console.log("Last played found.");
        var index = arg.files.indexOf(data.path);

        if (index != -1) {
          player = new Player(songArr);
        } else {
          player = new Player(songArr);
        }
      ...
      console.log("Last played Not found.");
      player = new Player(songArr);
      player.play();
      player.volume(0.5);
      // getTags(player.playlist[player.index].file);
      ...
      if (metadata.common.picture) {
        ...
        img.addEventListener("load", function () {
          if (theme == "disco") {
            ...
            if (swatches["DarkMuted"]) {
              let artistPill = document.body.querySelector("#artist");
              artistPill.style.backgroundColor = `hsl(${textHSL[0]} ${textHSL[1]}% ${textHSL[2]}%)`;
            } else
              document.body.querySelector("#artist").backgroundColor =
                "darkslategrey";
          }
        });
      } else {
...
var Player = function (playlist, index = 0) {
...
Player.prototype = {
  play: function (index) {
    ...
    if (data.howl) {
      ...
      console.log("getting data from howl");
